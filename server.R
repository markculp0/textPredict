...
# Create temp matrix for search results
m4 <- matrix(character(16),nrow=4,ncol=4)
alt1 <- ""
alt2 <- ""
alt3 <- ""
...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$predButton, { 
    ...
    # Add alternative word choices
    alt1 <- m4[2,3]
    alt2 <- m4[3,3]
    alt3 <- m4[4,3]
    
    # Set text box 1 output
    ...
    # Set text box 2 output
    output$out2 <- renderText({
      
      if (nrow(m4) > 3) {paste(alt1, "|", alt2, "|", alt3)
      } else if (nrow(m4) > 2){paste(alt1, "|", alt2)
      } else if (nrow(m4) > 1){paste(alt1)
      } else {paste("no alternative prediction")}
